...
!SUBROUTINE oasis_init_comp(mynummod,cdnam,kinfo,coupled,commworld)
 SUBROUTINE oasis_init_comp(mynummod,cdnam,kinfo,coupled,commworld,num_program_groups)
...
! kh 01.12.21
   INTEGER (kind=ip_intwp_p), intent(inout), optional :: num_program_groups
!  ---------------------------------------------------------
   integer(kind=ip_intwp_p) :: ierr
   integer(kind=ip_intwp_p) :: provided_thread_level
...
! kh 02.12.21
   integer(kind=ip_intwp_p) :: size_local_single
   integer(kind=ip_intwp_p) :: group_id
   integer(kind=ip_intwp_p) :: num_program_groups_local
...
!  ---------------------------------------------------------
   ...
   if(present(num_program_groups)) then
      num_program_groups_local = num_program_groups
   else
      num_program_groups_local = 1
   end if
   ...
   IF ( .NOT. lg_mpiflag ) THEN
      if (OASIS_debug >= 10) WRITE (0,FMT='(A)') subname//': Calling MPI_Init_thread'
      CALL MPI_INIT_THREAD ( MPI_THREAD_MULTIPLE, provided_thread_level, ierr )
      if (OASIS_debug >= 10) WRITE (0,FMT='(A,I0)') subname//': MPI thread support level: ', provided_thread_level
   else
...
! kh 01.12.21 resplit to handle multiple groups of same program
   ...
   size_local_single = mpi_size_local / num_program_groups_local
   group_id          = mpi_rank_local / size_local_single

!   write(*,*) '1st split: mpi_size_global, mpi_rank_global, mpi_size_local, mpi_rank_local, num_program_groups_local, size_local_single, group_id', &
!                          mpi_size_global, mpi_rank_global, mpi_size_local, mpi_rank_local, num_program_groups_local, size_local_single, group_id

   mpi_comm_global_all_groups = mpi_comm_global
   mpi_comm_local_all_groups  = mpi_comm_local
   
   call MPI_COMM_SPLIT(mpi_comm_global_all_groups, group_id,                   0, mpi_comm_global, ierr)
   call MPI_COMM_SPLIT(mpi_comm_global_all_groups, (compid * 1000) + group_id, 0, mpi_comm_local,  ierr)


! kh 02.12.21 debug info only
   CALL MPI_Comm_Size(mpi_comm_global,mpi_size_global,ierr)
   CALL MPI_Comm_Rank(mpi_comm_global,mpi_rank_global,ierr)
...
!   write(*,*) '2nd split: mpi_size_global, mpi_rank_global, mpi_size_local, mpi_rank_local', &
!                          mpi_size_global, mpi_rank_global, mpi_size_local, mpi_rank_local
...
!  ---------------------------------------------------------
   ...
   !------------------------
   !> * Synchronize all components before MPI finalize
   !------------------------
   
   IF ( .NOT. lg_mpiflag ) THEN
       ! Get current time for diagnostic output
       CALL DATE_AND_TIME(date_str, time_str)
       
       IF (OASIS_debug >= 2)  THEN
           WRITE (nulprt,FMT='(A,I4,A,A,A,A)') subname//': Component ', compid, &
                ' (', TRIM(compnm), ') reached finalization barrier at ', &
                date_str(1:4)//'-'//date_str(5:6)//'-'//date_str(7:8)//' '// &
                time_str(1:2)//':'//time_str(3:4)//':'//time_str(5:10)
           CALL oasis_flush(nulprt)
       ENDIF
       
       ! CRITICAL: All components must wait here before any calls MPI_Finalize
       WRITE (nulprt,FMT='(A,I4,A,A)') subname//': Component ', compid, &
            ' waiting at MPI_Barrier before MPI_Finalize at ', &
            time_str(1:2)//':'//time_str(3:4)//':'//time_str(5:10)
       CALL oasis_flush(nulprt)
       
       CALL MPI_Barrier(mpi_comm_global_world, ierr)
       
       CALL DATE_AND_TIME(date_str, time_str)
       WRITE (nulprt,FMT='(A,I4,A,A)') subname//': Component ', compid, &
            ' passed MPI_Barrier, calling MPI_Finalize at ', &
            time_str(1:2)//':'//time_str(3:4)//':'//time_str(5:10)
       CALL oasis_flush(nulprt)
       
       !------------------------
       !> * Write SUCCESSFUL RUN before MPI_Finalize
       !------------------------
       IF (OASIS_debug >= 2)  THEN
           CALL oasis_mem_print(nulprt,subname)
       ENDIF
       ...
       IF (OASIS_debug >= 2)  THEN
           WRITE (nulprt,FMT='(A)') subname//': Not Calling MPI_Finalize (MPI was already initialized)'
           CALL oasis_flush(nulprt)
       ENDIF
       close(nulprt)
   ENDIF
