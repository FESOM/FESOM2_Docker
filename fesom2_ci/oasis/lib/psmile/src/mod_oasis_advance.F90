    ...
    DO nc = 1,prism_var(varid)%ncpl
     ...
     if (runit) then
       ...
       if (comm_now) then
          ...
          ! Apply on both PUT and GET sides to support EXPOUT with LOCTRANS
          !------------------------------------------------

          if (getput == OASIS3_PUT .or. getput == OASIS3_GET) then
       ...
       IF (.not.lreadrest .and. &
           ((mseclag + dt >= maxtime .AND. &
...
!    if (mapper%nwgts == 4) higher_order_check = 1
    ...
    IF (prism_part(mapper%spart)%mpicom /= MPI_COMM_NULL) then
       ...
       ! if CONSERV option is GSMART, we use GLBPOS
       elseif (conserv == ip_cglbpos .or. conserv == ip_cgsmart) then
          ...
          do m = 1,fsize
             if (av_sumd(m) == 0.0_ip_r8_p .and. av_sums(m) /= 0.0_ip_r8_p) then
                ...
                ! if GSMART, then do GLOBAL this time step 
                ! i.e. we distribute difference between source sum and destination sum evenly over the points
                if (conserv == ip_cgsmart) then
                   WRITE(nulprt,*) subname,estr,'use GLOBAL this time step'
                   ! copy of GLOBAL method from above
                   if (wts_sumd == 0.0_ip_r8_p) then
                       WRITE(nulprt,*) subname,estr,'global masked area sums to zero '
                       call oasis_abort(file=__FILE__,line=__LINE__)
                   endif
                   ...
                   do n = 1,lsized
                      if (imaskd(n) == 0) avd%rAttr(m,n) = avd%rAttr(m,n) - zlagr
                   enddo
                ! if not GSMART, then throw error and stop
                else
                   call oasis_abort(file=__FILE__,line=__LINE__)
                end if
             else
