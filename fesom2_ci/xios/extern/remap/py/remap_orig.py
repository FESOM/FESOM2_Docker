...
if mode == 'remap':
	...
	src_val_glo = MPI.COMM_WORLD.gather(src_val_loc)
...
if rank == 0 and mode == 'remap':
	...
	f = nc.Dataset(outfile,'w')
	f.createDimension('n_vert', dst_nvert)
	f.createDimension('n_cell', dst_ncell)
	...
	var[:] = dst_val
	var = f.createVariable('val_ref', 'd', ('n_cell'))
	var.setncattr("coordinates", "lon lat")
	var[:] = dst_ref
	var = f.createVariable('err', 'd', ('n_cell'))
	var.setncattr("coordinates", "lon lat")
	var[:] = err
	var = f.createVariable('area', 'd', ('n_cell'))
	var.setncattr("coordinates", "lon lat")
	var[:] = dst_areas[:] # dest
	f.close()
