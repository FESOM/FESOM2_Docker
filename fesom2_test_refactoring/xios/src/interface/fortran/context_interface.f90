MODULE CONTEXT_INTERFACE
   ...
   INTERFACE ! Ne pas appeler directement/Interface FORTRAN 2003 <-> C99
      ...
      SUBROUTINE cxios_context_get_current(context) BIND(C)
         import C_INTPTR_T
         INTEGER (kind = C_INTPTR_T) :: context
      END SUBROUTINE cxios_context_get_current

      SUBROUTINE cxios_context_get_id(context, idt, idt_size) BIND(C)
         import C_CHAR, C_INTPTR_T, C_INT
         INTEGER (kind = C_INTPTR_T) :: context
         CHARACTER(kind = C_CHAR)    , DIMENSION(*) :: idt
         INTEGER  (kind = C_INT)     , VALUE        :: idt_size
     END SUBROUTINE cxios_context_get_id
